...
1.  **`user-service` (Port 8080):** Handles user authentication (login/registration), JWT generation, and serves the user dashboard.
2.  **`event-service` (Port 8081):** Manages the event lifecycle (CRUD operations) and serves the admin dashboard.
3.  **`registration-service` (Port 8082):** Manages the relationship between users and events (bookings).
